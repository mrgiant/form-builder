...
namespace Mrgiant\FormBuilder\Support;
...
class FilterQueryBuilder
{
            ...
            $query->where(function ($query) use ($data) {
                ...
                foreach ($data['f'] as $filter) {
                    ...
                    if ($this->isNestedColumn($filter['column'])) {
                        ...
                        $query->where($filter['column'], $likeOperator, '%'.$filter['query_1'].'%', $filter['match']);
    ...
    public function contains($filter, $query)
    {
        ...
        return $query->where($filter['column'], $likeOperator, '%'.$filter['query_1'].'%', $filter['match']);
    }
}
